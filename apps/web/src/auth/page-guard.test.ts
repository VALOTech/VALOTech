...
const { notFoundMock, redirectMock, requireAdminMock, requireHallReaderMock } = vi.hoisted(() => ({
  notFoundMock: vi.fn(() => {
  ...
  requireHallReaderMock: vi.fn(),
}));

vi.mock('next/navigation', () => ({ notFound: notFoundMock, redirect: redirectMock }));
vi.mock('next/headers', () => ({ headers: vi.fn(() => Promise.resolve(new Headers())) }));
vi.mock('./gate', () => ({ requireAdmin: requireAdminMock, requireHallReader: requireHallReaderMock }));

const { requireAdminPage, requireHallReaderPage } = await import('./page-guard');
...
describe('requireHallReaderPage', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('returns the actor when the gate admits an investor, touching neither escape', async () => {
    requireHallReaderMock.mockResolvedValue({ id: 'i1', role: 'investor' });

    await expect(requireHallReaderPage()).resolves.toEqual({ id: 'i1', role: 'investor' });
    expect(notFoundMock).not.toHaveBeenCalled();
    expect(redirectMock).not.toHaveBeenCalled();
  });

  it('admits an admin, who may read whatever an investor may', async () => {
    requireHallReaderMock.mockResolvedValue({ id: 'a1', role: 'admin' });

    await expect(requireHallReaderPage()).resolves.toEqual({ id: 'a1', role: 'admin' });
    expect(notFoundMock).not.toHaveBeenCalled();
    expect(redirectMock).not.toHaveBeenCalled();
  });

  it('redirects a signed-out reader to the sign-in form', async () => {
    requireHallReaderMock.mockResolvedValue(
      new Response(null, { status: 303, headers: { Location: '/sign-in' } }),
    );

    await expect(requireHallReaderPage()).rejects.toThrow('NEXT_REDIRECT:/sign-in');
    expect(redirectMock).toHaveBeenCalledWith('/sign-in');
    expect(notFoundMock).not.toHaveBeenCalled();
  });
});
