...
This product is **the company's own two surfaces**:
...
| `SITE-006` | Legal pages and the consent surface | building | `legal/privacy` and `legal/cookies` in twenty locales, linked from a legal row in the footer; `legal/terms` written and held for counsel; and the ecosystem's three-category consent banner with the non-essential categories off |
...
| `AUTH-005` | Registration | building | A second door, opened from the gateway by somebody considering an investment: a name and an address, and a confirmed link. It writes the `prospect` role, whose hall holds what the company publishes openly and whatever an admin grants by name (`AUTH-DEC-06`); an admin promotes them to an investor once they have invested. The door opens on `AUTH_REGISTRATION_OPEN`, which ships off so a deployment chooses when to accept registrations |
...
| `CMS-007` | Search and filter in the hall | live | Find an item by kind, by product, by period, or by its words. A hall with two years of updates and no search is an archive nobody reads |
