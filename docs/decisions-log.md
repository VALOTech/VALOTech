...
<a id="CMS-DEC-07"></a>
### `CMS-DEC-07` — What publish and withdraw do about a public item’s cache, when nothing the server can reach holds one — OPEN

- **Decision:** `CMS-004/T6` is "A public item’s cache is purged on publish and on withdraw", and two more designs say the same — `CMS-006` §3 ("a CDN may hold a copy of something that was public") and `POST-002` §3 ("Public updates are cached briefly and the cache is purged on publish and on withdraw"). `OPS-001` §3 settles the opposite for the edge: "Every route renders per request to reach the CSP nonce (`SEC-DEC-02`), so **Cloudflare caches the assets and never the documents** — a cached document is one reader’s answer served to the next", and `OPS-001/T8` is the task that makes that posture real. So there is no shared cache of a content document to purge, and the only cache that holds one is the reader’s own browser, for the ten minutes `apps/web/src/app/content/[id]/route.ts` grants it (`PUBLIC_MAX_AGE_SECONDS = 600`) — which a server cannot reach. What do publish and withdraw do instead?
- **Options:** **A** Keep the ten-minute lifetime and close `CMS-004/T6` as the bounded window it already is — no purge, because nothing purgeable exists; a withdrawn public document stays readable for up to ten more minutes to a browser that already fetched it, which is the window `CMS-006` audits rather than assumes away · **B** Serve a public document `no-cache` with an `ETag`, so a returning reader revalidates and a publish or a withdraw reaches them on their next request; the cost is a conditional request per repeat read, against an origin that renders every request anyway · **C** Build the purge — configure Cloudflare to cache documents and call its purge API on publish and withdraw, which needs an API credential and reverses `OPS-001` §3 as written.
- **Recommendation:** **B**, on one concrete ground rather than a preference: the withdrawal confirmation `RPT-002/T5` builds has to say what withdrawing does, and under **A** the honest sentence ends "… and a reader who already opened it may still see it for up to ten minutes", while under **B** it is simply that readers stop seeing it. Withdrawal is this room’s containment action — a report pulled, an audience narrowed by mistake — and a containment action whose confirmation has to disclose a residue is a weaker control than one without it. **A**’s case is genuinely strong and is not a straw option: the document was deliberately public, so ten more minutes of readability to somebody who already had it gives away nothing they could not have saved, the window is already audited and already bounded, and **B** buys a revalidation on every repeat read of every public document forever to close a rare event. What **A** does not answer is the admin who believes "withdrawn" means withdrawn. **C** is refused: it reverses a posture `OPS-001` derives from `SEC-DEC-02`’s per-request nonce, and buys an external credential to purge a cache the deploy is being built not to have.
- **Decision owner:** user
- **Blocks:** CMS-004/T6
- **Revises:** CMS-006/T6 — it shipped the ten-minute public lifetime as the bound on a narrowing window; **B** replaces that lifetime with revalidation
- **Status:** OPEN. Safe default: `public, max-age=600` on a public document and `private, no-store` on everything else, as shipped — bounded staleness rather than indefinite, with `apps/web/src/proxy.ts` overlaying `no-store` on any cookie-bearing request regardless, and `content.audience_change` recording the window rather than assuming it away (`CMS-006/T6`). Nothing is cached indefinitely under any of the three answers.

<a id="INFRA-DEC-08"></a>
### `INFRA-DEC-08` — Whether the console’s client components get a test environment, or stay browser-verified only — OPEN

- **Decision:** `apps/web/vitest.config.mts` sets `environment: 'node'` and the tree carries no DOM library, so nothing that only happens in a browser is exercised by a test: a keystroke, a drag, a focus move, a live-region announcement. Every client surface the console has shipped — the editor, the translation review, the destructive-action panel, the audience control, the deck overview’s reorder — is proven at the module and route level and verified in a browser by hand, or not at all. Only one of them has actually been driven, because three servers on this box share one `apps/web/.next`. Does the console gain a DOM test environment, or does it stay browser-verified by hand?
- **Options:** **A** Add `jsdom` plus `@testing-library/react` and `@testing-library/user-event`, and hold client components to the same standard as everything else — three development dependencies (§14), a second vitest project so the node suites keep their environment, and tests that exercise a keyboard and a pointer rather than a rendered string · **B** Keep the tree as it is and make the browser drive a real gate instead of an owed one, which needs a build window this machine does not currently have · **C** Add only `jsdom` and assert against the DOM directly, with no testing library — one dependency, and assertions written against markup rather than against what a person can do, which is the thing that keeps being wrong.
- **Recommendation:** **A**. What keeps going unverified is not rendering but behaviour — whether space picks a card up, whether escape puts it back, whether focus follows the thing that moved — and `user-event` is the only one of these options that drives those as a person does. **B**’s case is real and is not a straw option: a browser drive catches what no DOM shim can (a Content-Security-Policy that stops hydration, a CORS pair valid to `curl` and refused by every browser), and it is the instrument §10.8 was written around. It is rejected as the *only* answer rather than as an answer: one build window per surface does not scale to a console this size, and what it would gate is the run rather than the commit. **C** is refused because assertions written against markup are the ones that pass while the control is unusable.
- **Decision owner:** user
- **Blocks:** — none —
- **Revises:** DECK-001/T3 — it shipped with its move proven and its drag and key handling exercised by nothing; the answer decides whether that gap is closed by a test or by a drive
- **Status:** OPEN. Safe default: client components ship with their logic factored into pure, tested modules — the reorder is `content/sections.ts:reorderSections`, the audience sentences are `content/audience.ts:audienceOptions` — so what a component holds is wiring, and every surface carries its browser drive as owed work named in the iteration log. Nothing is claimed as verified that is not.
...
- **Decision owner:** user
- **Settled by:** user
- **Status:** RESOLVED 2026-09-13 — **A**, and the answer turned out to be **B**'s shape rather than a number. The grounds between the reader and the world *multiply*: a scrim dark enough to protect every word, with a panel over it, leaves `(1 − panel) × (1 − scrim)` of the world on the screen, and a first pass at 0.74 over 0.72 left **5%** — every run of copy at AA, and a world the owner could not see at all, which is the goal missed while the constraint was met. So the scrim became a wash (0.28) and the blocks standing on it carry their own plate, which means only two layers ever stack. The plate is glass rather than paint: what fights a letterform is the world's *detail*, not its brightness, so a blurred backdrop buys the same legibility at far less opacity — everywhere except the cover, where the world is the bare lunar rock whose craters are its only signal and a blur erased it at 14px and still at 5px. The small mono labels then bound the rest through their colour, and one card family through the colour of its ground: an accent label needs a card at 0.72 where body text needs 0.55, and the glass family's indigo fill is lighter than the card family's near-black by enough to cost 2.4:1. So the labels carry the bright accent, the glass card that stands on the brightest part of the world takes the dark ground, and the cards stay open at 0.62. Measured: **27% of the world behind a card, 42% under a plate, 40% under the cover's, 72% on the bare wash**, against 0.4% before this session, with **0 of 900 runs below AA** at 390, 360 and 320. **C** would have narrowed `A11Y-R03`, which §1.11 reserves.

<a id="SCENE-DEC-03"></a>
### `SCENE-DEC-03` — Whether the journey is read ahead of the reader — RESOLVED 2026-09-13 · loop-settled

- **Decision:** The scene eased the world toward its station, so it trailed the station by the station's speed times the easing's time constant, and a chapter could be reached before the world was in the space that chapter left. `SCENE-001` answered that by reading the journey a little ahead of the reader, on the reasoning that the lead and the trail are the same product and subtract. The owner reported the world jumping and then settling back, and feeling loose rather than placed. Does the read-ahead stay?
- **Options:** **A** Remove it — the world follows exactly where the reader is · **B** Keep it, bounded by a share of the leg rather than of the page · **C** Keep it as written and accept the motion.
- **Decision owner:** user — settled by the loop under `.claude/CLAUDE.md` §1.11, reversible by reverting one commit
- **Settled by:** loop
- **Forcing source:** MEASUREMENT `assets/scene/boot.js:place` — the premise is false as written. The trail is a distance on the screen and the read-ahead is a step along the journey, and the journey maps the two through a different slope on every leg, so they cannot subtract except by coincidence. Driven with a real wheel over fourteen segments of the page at 1440 x 900: the world's worst travel *against its own direction* measured **133.9px across the frame and 45.5px down** with the read-ahead as written, **56.8 and 29.9** under **B**, and **26.9 and 6.8** under **A**, with twelve of the fourteen segments then under a pixel. Against that, the arrival it was written to buy measured **6.8px versus 6.6px median and 420ms versus 410ms** between **B** and **A** — inside the noise, with **A** worse only on the worst single chapter, by 2.3px. **C**'s strongest case is that the read-ahead was written against a real fault and removing it returns that fault; it does not survive, because the fault it returns measures two pixels and the one it removes measures thirty, and only one of the two is something a reader can see.
- **Overturned by:** a station table whose legs are close enough in slope that a page-fraction lead and a screen distance are the same quantity, or a reader reporting the world arriving late at a chapter.
- **Status:** RESOLVED 2026-09-13 — **A**. The read-ahead and its four constants are gone; the journey is read where the reader is. The trail remains, which is a world following rather than a world snapping, and is what the easing was for. Resolved together with the idle drift, which is `SCENE-001/T9`: two sinusoids moved the disc 5.9px across the frame and 11.2px down it with the page standing still and the journey settled, against `SCENE-R03`, which had been applied below 900px only. At rest the world now measures 0.00px at three points of the page and at both widths.
