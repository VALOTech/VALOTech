...
- **Decision owner:** user
- **Settled by:** user
- **Status:** RESOLVED 2026-09-12 — **A**, `jimp` and refuse SVG. Raster uploads are re-encoded through `jimp` (pure JavaScript, no native build, so the Docker and deploy targets stay simple), which drops EXIF and its GPS by re-writing the pixels; SVG is refused, the design's own stated fallback (`CMS-003` §6), which adds no sanitiser and removes the format most likely to carry script, at the cost of a logo arriving as PNG. `CMS-003/T2` and `CMS-003/T3` build on this one pure-JavaScript dependency. Reopen for `sharp` and an SVG sanitiser if a real SVG need arrives or the image volume outgrows what `jimp` serves.
